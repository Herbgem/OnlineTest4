<%@ Page Title="" Language="C#" MasterPageFile="~/Site1.Master" AutoEventWireup="true" CodeBehind="Home.aspx.cs" Inherits="OnlineTest4.WebForm1" %>

<%@ Register Src="~/CustomUserControl/HeaderUC.ascx" TagPrefix="uc1" TagName="HeaderUC" %>




<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="HeaderRegion" runat="server">
</asp:Content>
<asp:Content ID="Content3" ContentPlaceHolderID="ContentRegion" runat="server">
    <h1 style="position:relative; text-align:center; margin-top:1em;">Welcom to O!Test</h1>
</asp:Content>
